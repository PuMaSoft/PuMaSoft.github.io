...
jQuery(document).ready(function () {
    ...
    $('form#form').submit(function () {
        ...
        if (!hasError) {
            ...
            $.post($(this).attr('action'), formInput, function (data) {
                $('form#form').append('<div class="success"><div class="col-md-12"><div class="alert alert-nesto"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><p>Dziękujemy za kontakt! Twoja wiadomość została wysłana.</p></div></div></div>');
            });
            $('.requiredField').val('');
        }
        return false;
    ...
    map = new GMaps({
        el: '#map',
        scrollwheel: false,
        lat: 52.297813, // 52.297813
        lng: 21.150000  // 21.158270
    });
    map.addMarker({
        lat: 52.297813,
        lng: 21.161000,
...
function showEmailAdr() {
 var at = '@pumasoft';
 var pre = 'biuro';
 var full = pre + at + '.com.pl';
 var adr = prompt("Teraz możesz skopiować adres e-mail, aby wkleić go w programie pocztowym:", full);
};
